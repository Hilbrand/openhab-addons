...
public class DeviceTestBase {
    ...
    /**
     * Sets the answers to return when the socket.getInputStream() is requested. If multiple files are given they will
     * returned in order each time a call to socket.getInputStream() is done.
     *
     * @param responseFilenames name of the files to read that contains the answer. It's the unencrypted json string
     * @throws IOException exception in case device not reachable
     */
