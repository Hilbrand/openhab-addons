...
public class UniFiPoePortThingHandler
    ...
    private boolean handleModeCommand(final UniFiController controller, final Map<Integer, UniFiPortTuple> ports,
        ...
        if (device == null || uniFiPortTuple == null) {
            ...
            // Safety check to make sure the same amount of ports to avoid corrupting override data on the device.
            if (updatedList.size() != ports.size()) {
                logger.info(
                        "Did not update PoE port for thing '{}' state as data doesn't seem to be correct. Data to use for update was: {}",
                        getThing().getUID(), updatedList);
            } else {
                updatedList.stream().filter(p -> p.getPortIdx() == uniFiPortTuple.getPortIdx()).findAny()
                        .ifPresent(p -> p.setPoeMode(poeMode));

                return controller.poeMode(device, updatedList);
